...
before openpyxl writes anything. Every applied or rejected mutation is
audited; idempotent replays are skipped without side effects.
"""
...
def apply_mutations(draft_path, mutations, schema, allowlist, audit, run_id):
    ...
    outcomes = [
        _apply_one(workbook, mutation, schema, allowlist, audit, run_id)
...
def _apply_one(workbook, mutation, schema, allowlist, audit, run_id):
    def rejected(reason):
        return MutationOutcome(mutation=mutation, status="rejected", reason=reason)
    ...
    if not writer.has_sheet(workbook, mutation.sheet):
        return rejected(f"sheet {mutation.sheet!r} is not present in the workbook")
    ...
    if field is None:
        return rejected(f"no field declared for column {column}")

    if not field.writable:
        return rejected(f"column {column} ({header}) is not writable")
    ...
    if not allowlist.permits(mutation.sheet, cell_ref):
        return rejected(f"cell {mutation.sheet}!{cell_ref} is not in the allowlist")

    prior = audit.find_applied_mutation(
        run_id, mutation.sheet, cell_ref, mutation.actor_role, mutation.source_ref
    )
    if prior is not None:
        ...
        return MutationOutcome(mutation=mutation, status="skipped")
    ...
    return MutationOutcome(mutation=mutation, status="applied", old_value=old_value)


def _record(outcome):
    mutation = outcome.mutation
    cell_ref = writer.normalize_cell(mutation.cell) or mutation.cell
    return {
